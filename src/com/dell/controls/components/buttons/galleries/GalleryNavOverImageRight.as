...
public class GalleryNavOverImageRight extends DefaultButton {

    //Callout
    private var _callout:C_Callout_Default;

	
    public function GalleryNavOverImageRight( name:String, vars:Object = null, data:Object = null) {
...
//        _offGraphic = new MovieClipFactory( new UIGalleryNav().mcGalleryRightNavOFF );
		_offGraphic = new MovieClipFactory( new UIGalleryNav().mcGalleryRightNavON );
        this.addGraphic( _offGraphic );

        _onGraphic = new MovieClipFactory( new UIGalleryNav().mcGalleryRightNavON );
        this.addGraphic( _onGraphic );

        _overGraphic = new MovieClipFactory( new UIGalleryNav().mcGalleryRightNavON );
        this.addGraphic( _overGraphic );

        const params:AbstractCalloutParams = new GalleryRightLeftNavCalloutButtonParams();
        params.text = Global.data.copy.screen[ Global.moduleIndex ].slides[ 1 ].headline;
//        params.text = "DEFAULT TEXT";
        _callout = new Callout_C_Factory().add( Callout_C_Factory.DEFAULT_CALLOUT, this, 0, 0, params ) as C_Callout_Default;
